...
if (requireNamespace("HiContacts", quietly = TRUE)) {
    ps <- HiContacts::distanceLaw(hic, by_chr = TRUE) |> suppressMessages()
    fig_ps <- plotly::plot_ly(
        ps, 
    ...
    cistrans <- HiContacts::cisTransRatio(hic)
    fig_cistrans <- plotly::plot_ly(
        cistrans, 
        x = ~chr, y = ~cis_pct*100, 
        type = 'bar', 
        name = 'Cis interactions'
    ) |> 
        plotly::add_trace(
            y = ~trans_pct*100, name = 'Trans interactions'
        ) |> 
        plotly::layout(
            barmode = 'stack', 
            title = paste0(
                "Proportion of cis interactions within each chromosome"
            ), 
            xaxis = list(
                title = 'Chromosome'
            ),
            yaxis = list(
                title = '% of cis interactions'
            )
        )
...
if (requireNamespace("HiContacts", quietly = TRUE)) {
    ...
    message("Install HiContacts package to compute distance law.")
    message("BiocManager::install('HiContacts')")
}
```

> Cis/trans interaction frequencies

```{r}
if (requireNamespace("HiContacts", quietly = TRUE)) {
    fig_cistrans
} else {
    message("Install HiContacts package to compute cis/trans interaction frequencies.")
    message("BiocManager::install('HiContacts')")
}
```
